...
import { Building2, DollarSign, Calendar, Pencil, Trash2 } from "lucide-react";
...
export default function BankAccountCard({ account, onEdit, onDelete }) {
  ...
  useEffect(() => {
    // If this is a business account, load the business data
    if (account.account_type !== 'personal' && account.business_id) {
      loadBusinessData();
    }
  }, [account]);
  ...
  const loadBusinessData = async () => {
    try {
      ...
      const business = businessesData.find(b => b.id === account.business_id);
                  ...
                  <>
                    ...
                    {account.account_type === 'business' ? (
                      ...
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                        Mixed ({account.business_use_percentage}% business)
                      </span>
                    )}
                  </>
                )}
